...
`set basketItems(items: HTMLElement[])` - устанавливает и отображает список товаров в корзине
`buttonActivate(itemsNumber: number)` - активация или деактивация кнопки дальнейшего оформления закказа
...
`activateBuyButton` - включение или выключение кнопки добавления в корзину
...
`get errors()` - получить значения ошибок 
...
Методы класса:
`activateNextButton(switcher:boolean)` - включение или отключение кнопки дальнейшего оформления заказа
...
`open()` - изменение списка классов элемента для его отображения на форме
`close()` - изменение списка классов элемента для его скрытия с формы
...
# Презентер
Класс написан в файле main.ts. Отвечает за координирование работы всех слоев и классов проекта

Конструктор:
`constructor()` - не требует аргументов, создает все экземпляры задействованных в работе программы классов, устанавливает все реализации обработки событий

Поля: 
`private basketView: BasketView` - отображение корзины
`private cardPreview: CardPreview` - отображение полной версии карточки товара
`private formOrder: FormOrder` - отображение формы заполнения данных заказа
`private formContact: FormContact`- отображение формы заполнения данных с контактами
`private gallery: Gallery` - отображение каталога товаров
`private header: Header` - отображение шапки сайта
`private modalContainer: ModalContainer`- отображение модального окна
`private success: Success` - отображение окна подтверждения заказа
`private basket: Basket` - модель данных корзины
`private buyer: Buyer` - модель данных покупателя
`private catalog: Catalogue` - модель данных каталога товаров
`private apiInteraction: ApiInteraction` - модель связи с сервером
`private api: Api` - базовый апи
`private events: EventEmitter` - обработчик событий
